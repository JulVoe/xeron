#ifndef APEX_MEMMOVE_CPP
#define APEX_MEMMOVE_CPP
#include "apex_memmove.hpp"
#include "../SystemInfo/Makros.hpp"        //For Compiler-Makros
#if defined(DYNAMIC_TEST)
#include "../SystemInfo/DynamicInfo.hpp"   //For __get_cpuid
#endif

#include <cstring>						// For std::memmove, std::memcpy as a fallback
#include <x86intrin.h>
...
namespace apex
{
	...
	#if defined(INTEL) || defined(AMD)	//	Test for Intel/AMD architecture
	...
	#if BIT==32					//	`mithril` is only used in 32-bit code (not necessary for 64-bit)
	...
	void* APEXCALL memmove_dispatcher( void *dst, const void *src, size_t size )
	{
#if defined(DYNAMIC_TEST)
			#if BIT==64					//	64-bit
				if ( sysinfo::supportsSSE4_2() )							//	detect SSE4.2, available on Core i and newer processors, they include "fast unaligned" memory access
				...
				if ( sysinfo::supportsSSE4_2() )							//	detect SSE4.2, available on Core i and newer processors, they include "fast unaligned" memory access
					apex::memcpy = apex::memmove = &apex::kryptonite;
				else if ( sysinfo::supportsSSSE3() )						//	detect SSSE3, available on Core/Core 2 and newer
...
#else
#if BIT==64					//	64-bit
#if SSE==42							//	detect SSE4.2, available on Core i and newer processors, they include "fast unaligned" memory access
					apex::memcpy = apex::memmove = &apex::kryptonite;
#else
					apex::memcpy = apex::memmove = &apex::tiberium;
#endif
...
#if SSE==42							//	detect SSE4.2, available on Core i and newer processors, they include "fast unaligned" memory access
					apex::memcpy = apex::memmove = &apex::kryptonite;
#elif SSE>=35						//	detect SSSE3, available on Core/Core 2 and newer
					apex::memcpy = apex::memmove = &apex::tiberium;		//	`tiberium` CAN run with the SSE2 instructions alone, however there was something about very old SSE2 (only) computers, some penalty when using `tiberiums` algorithm, so I've restricted `tiberium` to Core/Core 2 based machines on 32-bit by doing this!
#elif SSE>=2
					apex::memcpy = apex::memmove = &apex::mithril;		//	this is for very, very old computers with SSE2 only! eg. old Pentium 4! There was something about computers WITHOUT SSSE3 (Core microarchitecture) that made `tiberium` a bit slower. So this is for super old (P4) PC's!
#else
