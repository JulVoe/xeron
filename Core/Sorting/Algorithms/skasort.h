...
struct SizedRadixSorter<2>
{
    ...
    //This is and LSD Radix sort with radix 256, but it does 2 Iterations at once. Also, a offset array is not needed, howerver, a buffer unfortiontly is.
    template<typename count_type, typename It, typename OutIt, typename ExtractKey>
    static bool sort_inline(It begin, It end, OutIt out_begin, OutIt out_end, ExtractKey && extract_key)
    {
        ...
        for (It it = begin; it != end; ++it)
        {
            ...
            ++counts0[key & 0xff];//LSByte
            ++counts1[(key >> 8) & 0xff];//2.LSByte
        }
        count_type total0 = 0;
        count_type total1 = 0;
        for (int i = 0; i < 256; ++i) //Computes the offset
        {
...
struct SizedRadixSorter<4>
{
    ...
    //An LSD Radix sortwith radix 256 and 4 iterations at a time. An offset array is not requiered, but a buffer is.
    template<typename count_type, typename It, typename OutIt, typename ExtractKey>
    static bool sort_inline(It begin, It end, OutIt out_begin, OutIt out_end, ExtractKey && extract_key)
    {

        count_type* counts0 = (count_type*)calloc(256,sizeof(count_type));
        count_type* counts1 = (count_type*)calloc(256,sizeof(count_type));
        count_type* counts2 = (count_type*)calloc(256,sizeof(count_type));
        count_type* counts3 = (count_type*)calloc(256,sizeof(count_type));
...
inline void StdSortFallback(It begin, It end, ExtractKey & extract_key)
{
	...
	semi_static_sort(begin,end-begin);
}
