...
struct SizedRadixSorter<4>
{
    ...
    static bool sort_inline(It begin, It end, OutIt out_begin, OutIt out_end, ExtractKey && extract_key)
    {
        ...
        for (int i = 0; i < 256; ++i)
        {
        	size_t old_count0 = counts0[i];
        	size_t old_count1 = counts1[i];
        	size_t old_count2 = counts2[i];
        	size_t old_count3 = counts3[i];
            counts0[i] = total0;
            counts1[i] = total1;
            counts2[i] = total2;
            counts3[i] = total3;
        ...
        for (OutIt it = out_begin; it != out_end; ++it)
        {
            std::uint8_t key = *it >> 8;
            begin[counts1[key]++] = std::move(*it);
        }
        for (It it = begin; it != end; ++it)
        {
            std::uint8_t key = *it >> 16;
            out_begin[counts2[key]++] = std::move(*it);
        }
        for (OutIt it = out_begin; it != out_end; ++it)
        {
            std::uint8_t key = *it >> 24;
